...
@ManagedBean
@SessionScoped
public class PaisPersistencia implements Serializable {
private static final long serialVersionUID = 1L;
public static Exception excepcion;
public static Connection con = null;

public PaisPersistencia(){
...
public static boolean Insertar (Pais pais ) {
	...
	String sql = "insert into pais(id_pais,nombre,codigo_area,siglas)  values('"+pais.getId_pais()+"','"+pais.getNombre()+"','"+pais.getCodigo_area()+"','"+pais.getSiglas()+"')";
...
public static List<Pais> getLista(Map parametros) {
  excepcion = null;
  try {
      if (!Conectar()) {
         return null;
  ...
  PreparedStatement psquery = con.prepareStatement(sql,ResultSet.TYPE_SCROLL_INSENSITIVE,
   ...
   while (rs.next()) {
       list.add(new Pais(
         rs.getInt("id_pais"),rs.getString("nombre"),rs.getString("codigo_area"),rs.getString("siglas")));
    }
    rs.close();
    psquery.close();
    con.close();
