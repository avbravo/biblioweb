...
@ManagedBean
@SessionScoped
public class Cuenta_tipoPersistencia implements Serializable {
private static final long serialVersionUID = 1L;
public static Exception excepcion;
public static Connection con = null;

public Cuenta_tipoPersistencia(){
     ELContext elContext = FacesContext.getCurrentInstance().getELContext();
}
public static boolean Insertar (Cuenta_tipo cuenta_tipo ) {
	excepcion = null;
	try {
	if (!Conectar()) {
	return false;
	}
	String sql = "insert into cuenta_tipo(id_cuetip,nombre)  values('"+cuenta_tipo.getId_cuetip()+"','"+cuenta_tipo.getNombre()+"')";
...
public static List<Cuenta_tipo> getLista(Map parametros) {
  excepcion = null;
  try {
      if (!Conectar()) {
         return null;
      }
  List<Cuenta_tipo> list = new ArrayList<Cuenta_tipo>();
  String sql = "select * from cuenta_tipo";
  if (parametros != null) {
  ...
  PreparedStatement psquery = con.prepareStatement(sql,ResultSet.TYPE_SCROLL_INSENSITIVE,
   ...
   rs.beforeFirst();
   while (rs.next()) {
       list.add(new Cuenta_tipo(
         rs.getInt("id_cuetip"),rs.getString("nombre")));
    }
    rs.close();
    psquery.close();
    con.close();
