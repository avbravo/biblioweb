...
@ManagedBean
@SessionScoped
public class ProveedorPersistencia implements Serializable {
...
public static boolean Insertar (Proveedor proveedor ) {
	...
	String sql = "insert into proveedor(id_proveedor,nombre,telefono,email,fax,direccion)  values('"+proveedor.getId_proveedor()+"','"+proveedor.getNombre()+"','"+proveedor.getTelefono()+"','"+proveedor.getEmail()+"','"+proveedor.getFax()+"','"+proveedor.getDireccion()+"')";
...
public static List<Proveedor> getLista(Map parametros) {
  excepcion = null;
  try {
      if (!Conectar()) {
         return null;
  ...
  PreparedStatement psquery = con.prepareStatement(sql,ResultSet.TYPE_SCROLL_INSENSITIVE,
   ...
   while (rs.next()) {
       list.add(new Proveedor(
         rs.getInt("id_proveedor"),rs.getString("nombre"),rs.getString("telefono"),rs.getString("email"),rs.getString("fax"),rs.getString("direccion")));
    }
    rs.close();
    psquery.close();
    con.close();
