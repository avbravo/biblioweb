...
@ManagedBean
@SessionScoped
public class UrlPersistencia implements Serializable {
private static final long serialVersionUID = 1L;
public static Exception excepcion;
public static Connection con = null;

public UrlPersistencia(){
     ELContext elContext = FacesContext.getCurrentInstance().getELContext();
}
public static boolean Insertar (Url url ) {
	excepcion = null;
	try {
	if (!Conectar()) {
	return false;
	}
	String sql = "insert into url(id_url,nombre,url)  values('"+url.getId_url()+"','"+url.getNombre()+"','"+url.getUrl()+"')";
...
public static List<Url> getLista(Map parametros) {
  excepcion = null;
  try {
      if (!Conectar()) {
         return null;
  ...
  PreparedStatement psquery = con.prepareStatement(sql,ResultSet.TYPE_SCROLL_INSENSITIVE,
   ...
   while (rs.next()) {
       list.add(new Url(
         rs.getInt("id_url"),rs.getString("nombre"),rs.getString("url")));
    }
    rs.close();
    psquery.close();
    con.close();
