...
@ManagedBean
@SessionScoped
public class InteresPersistencia implements Serializable {
private static final long serialVersionUID = 1L;
public static Exception excepcion;
public static Connection con = null;

public InteresPersistencia(){
     ELContext elContext = FacesContext.getCurrentInstance().getELContext();
}
public static boolean Insertar (Interes interes ) {
	excepcion = null;
	try {
	if (!Conectar()) {
	return false;
	}
	String sql = "insert into interes(id_interes,nombre,descripcion,url)  values('"+interes.getId_interes()+"','"+interes.getNombre()+"','"+interes.getDescripcion()+"','"+interes.getUrl()+"')";
...
public static List<Interes> getLista(Map parametros) {
  excepcion = null;
  try {
      if (!Conectar()) {
         return null;
      }
  List<Interes> list = new ArrayList<Interes>();
  ...
  PreparedStatement psquery = con.prepareStatement(sql,ResultSet.TYPE_SCROLL_INSENSITIVE,
   ...
   rs.beforeFirst();
   while (rs.next()) {
       list.add(new Interes(
         rs.getInt("id_interes"),rs.getString("nombre"),rs.getString("descripcion"),rs.getString("url")));
    }
    rs.close();
    psquery.close();
    con.close();
