...
@ManagedBean
@SessionScoped
public class Marc_seriePersistencia implements Serializable {
private static final long serialVersionUID = 1L;
public static Exception excepcion;
public static Connection con = null;

public Marc_seriePersistencia(){
     ELContext elContext = FacesContext.getCurrentInstance().getELContext();
}
public static boolean Insertar (Marc_serie marc_serie ) {
	excepcion = null;
	try {
	if (!Conectar()) {
	return false;
	}
	String sql = "insert into marc_serie(id_serie,id_dato,nombre)  values('"+marc_serie.getId_serie()+"','"+marc_serie.getId_dato()+"','"+marc_serie.getNombre()+"')";
...
public static List<Marc_serie> getLista(Map parametros) {
  excepcion = null;
  try {
      if (!Conectar()) {
         return null;
      }
  List<Marc_serie> list = new ArrayList<Marc_serie>();
  String sql = "select * from marc_serie";
  if (parametros != null) {
  ...
  PreparedStatement psquery = con.prepareStatement(sql,ResultSet.TYPE_SCROLL_INSENSITIVE,
   ...
   rs.beforeFirst();
   while (rs.next()) {
       list.add(new Marc_serie(
         rs.getInt("id_serie"),rs.getInt("id_dato"),rs.getString("nombre")));
    }
    rs.close();
    psquery.close();
    con.close();
