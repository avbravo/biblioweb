...
@ManagedBean
@SessionScoped
public class EstatusPersistencia implements Serializable {
private static final long serialVersionUID = 1L;
public static Exception excepcion;
public static Connection con = null;

public EstatusPersistencia(){
     ELContext elContext = FacesContext.getCurrentInstance().getELContext();
}
public static boolean Insertar (Estatus estatus ) {
	excepcion = null;
	try {
	if (!Conectar()) {
	return false;
	}
	String sql = "insert into estatus(id_estatus,id_modulo,nombre,habilitado)  values('"+estatus.getId_estatus()+"','"+estatus.getId_modulo()+"','"+estatus.getNombre()+"','"+estatus.getHabilitado()+"')";
...
public static List<Estatus> getLista(Map parametros) {
  excepcion = null;
  try {
      if (!Conectar()) {
         return null;
  ...
  PreparedStatement psquery = con.prepareStatement(sql,ResultSet.TYPE_SCROLL_INSENSITIVE,
   ...
   while (rs.next()) {
       list.add(new Estatus(
         rs.getInt("id_estatus"),rs.getInt("id_modulo"),rs.getString("nombre"),rs.getBoolean("habilitado")));
    }
    rs.close();
    psquery.close();
    con.close();
