...
@ManagedBean
@SessionScoped
public class MensajePersistencia implements Serializable {
private static final long serialVersionUID = 1L;
public static Exception excepcion;
public static Connection con = null;

public MensajePersistencia(){
     ELContext elContext = FacesContext.getCurrentInstance().getELContext();
}
public static boolean Insertar (Mensaje mensaje ) {
	excepcion = null;
	try {
	if (!Conectar()) {
	return false;
	}
	String sql = "insert into mensaje(id_mensaje,titulo,contenido,fecha,hora)  values('"+mensaje.getId_mensaje()+"','"+mensaje.getTitulo()+"','"+mensaje.getContenido()+"','"+mensaje.getFecha()+"','"+mensaje.getHora()+"')";
...
public static List<Mensaje> getLista(Map parametros) {
  excepcion = null;
  try {
      if (!Conectar()) {
         return null;
  ...
  PreparedStatement psquery = con.prepareStatement(sql,ResultSet.TYPE_SCROLL_INSENSITIVE,
   ...
   while (rs.next()) {
       list.add(new Mensaje(
         rs.getInt("id_mensaje"),rs.getString("titulo"),rs.getString("contenido"),rs.getDate("fecha"),rs.getTime("hora")));
    }
    rs.close();
    psquery.close();
    con.close();
