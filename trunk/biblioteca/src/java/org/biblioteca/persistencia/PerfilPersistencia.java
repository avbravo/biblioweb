...
@ManagedBean
@SessionScoped
public class PerfilPersistencia implements Serializable {
...
public static boolean Insertar (Perfil perfil ) {
	...
	String sql = "insert into perfil(id_perfil,nombre)  values('"+perfil.getId_perfil()+"','"+perfil.getNombre()+"')";
...
public static List<Perfil> getLista(Map parametros) {
  excepcion = null;
  try {
      if (!Conectar()) {
         return null;
  ...
  PreparedStatement psquery = con.prepareStatement(sql,ResultSet.TYPE_SCROLL_INSENSITIVE,
   ...
   while (rs.next()) {
       list.add(new Perfil(
         rs.getInt("id_perfil"),rs.getString("nombre")));
    }
    rs.close();
    psquery.close();
    con.close();
