...
@ManagedBean
@SessionScoped
public class FrecuenciaPersistencia implements Serializable {
private static final long serialVersionUID = 1L;
public static Exception excepcion;
public static Connection con = null;

public FrecuenciaPersistencia(){
     ELContext elContext = FacesContext.getCurrentInstance().getELContext();
}
public static boolean Insertar (Frecuencia frecuencia ) {
	excepcion = null;
	try {
	if (!Conectar()) {
	return false;
	}
	String sql = "insert into frecuencia(id_frecuencia,nombre)  values('"+frecuencia.getId_frecuencia()+"','"+frecuencia.getNombre()+"')";
...
public static List<Frecuencia> getLista(Map parametros) {
  excepcion = null;
  try {
      if (!Conectar()) {
         return null;
  ...
  PreparedStatement psquery = con.prepareStatement(sql,ResultSet.TYPE_SCROLL_INSENSITIVE,
   ...
   rs.beforeFirst();
   while (rs.next()) {
       list.add(new Frecuencia(
         rs.getInt("id_frecuencia"),rs.getString("nombre")));
    }
    rs.close();
    psquery.close();
    con.close();
