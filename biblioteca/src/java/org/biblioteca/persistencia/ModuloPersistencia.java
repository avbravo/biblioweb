...
@ManagedBean
@SessionScoped
public class ModuloPersistencia implements Serializable {
...
public static boolean Insertar (Modulo modulo ) {
	...
	String sql = "insert into modulo(id_modulo,nombre)  values('"+modulo.getId_modulo()+"','"+modulo.getNombre()+"')";
...
public static List<Modulo> getLista(Map parametros) {
  excepcion = null;
  try {
      if (!Conectar()) {
         return null;
  ...
  PreparedStatement psquery = con.prepareStatement(sql,ResultSet.TYPE_SCROLL_INSENSITIVE,
   ...
   while (rs.next()) {
       list.add(new Modulo(
         rs.getInt("id_modulo"),rs.getString("nombre")));
    }
    rs.close();
    psquery.close();
    con.close();
