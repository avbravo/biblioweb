...
@ManagedBean
@SessionScoped
public class ControlPersistencia implements Serializable {
...
public static boolean Insertar (Control control ) {
	excepcion = null;
	try {
	if (!Conectar()) {
	return false;
	}
	String sql = "insert into control(id_control,nombre)  values('"+control.getId_control()+"','"+control.getNombre()+"')";
...
public static List<Control> getLista(Map parametros) {
  excepcion = null;
  try {
      if (!Conectar()) {
         return null;
  ...
  PreparedStatement psquery = con.prepareStatement(sql,ResultSet.TYPE_SCROLL_INSENSITIVE,
   ...
   while (rs.next()) {
       list.add(new Control(
         rs.getInt("id_control"),rs.getString("nombre")));
    }
    rs.close();
    psquery.close();
    con.close();
