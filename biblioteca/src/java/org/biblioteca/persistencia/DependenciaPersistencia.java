...
@ManagedBean
@SessionScoped
public class DependenciaPersistencia implements Serializable {
private static final long serialVersionUID = 1L;
public static Exception excepcion;
public static Connection con = null;

public DependenciaPersistencia(){
     ELContext elContext = FacesContext.getCurrentInstance().getELContext();
}
public static boolean Insertar (Dependencia dependencia ) {
	excepcion = null;
	try {
	if (!Conectar()) {
	return false;
	}
	String sql = "insert into dependencia(id_dependencia,id_institucion,nombre)  values('"+dependencia.getId_dependencia()+"','"+dependencia.getId_institucion()+"','"+dependencia.getNombre()+"')";
...
public static List<Dependencia> getLista(Map parametros) {
  excepcion = null;
  try {
      if (!Conectar()) {
         return null;
  ...
  PreparedStatement psquery = con.prepareStatement(sql,ResultSet.TYPE_SCROLL_INSENSITIVE,
   ...
   while (rs.next()) {
       list.add(new Dependencia(
         rs.getInt("id_dependencia"),rs.getInt("id_institucion"),rs.getString("nombre")));
    }
    rs.close();
    psquery.close();
    con.close();
