...
@ManagedBean
@SessionScoped
public class RegionPersistencia implements Serializable {
private static final long serialVersionUID = 1L;
public static Exception excepcion;
public static Connection con = null;

public RegionPersistencia(){
...
public static boolean Insertar (Region region ) {
	...
	String sql = "insert into region(id_region,id_pais,nombre)  values('"+region.getId_region()+"','"+region.getId_pais()+"','"+region.getNombre()+"')";
...
public static List<Region> getLista(Map parametros) {
  excepcion = null;
  try {
      if (!Conectar()) {
         return null;
  ...
  PreparedStatement psquery = con.prepareStatement(sql,ResultSet.TYPE_SCROLL_INSENSITIVE,
   ...
   while (rs.next()) {
       list.add(new Region(
         rs.getInt("id_region"),rs.getInt("id_pais"),rs.getString("nombre")));
    }
    rs.close();
    psquery.close();
    con.close();
