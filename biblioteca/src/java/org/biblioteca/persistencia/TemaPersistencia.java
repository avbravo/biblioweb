...
@ManagedBean
@SessionScoped
public class TemaPersistencia implements Serializable {
private static final long serialVersionUID = 1L;
public static Exception excepcion;
public static Connection con = null;

public TemaPersistencia(){
     ELContext elContext = FacesContext.getCurrentInstance().getELContext();
}
public static boolean Insertar (Tema tema ) {
	excepcion = null;
	try {
	if (!Conectar()) {
	return false;
	}
	String sql = "insert into tema(id_tema,nombre)  values('"+tema.getId_tema()+"','"+tema.getNombre()+"')";
...
public static List<Tema> getLista(Map parametros) {
  excepcion = null;
  try {
      if (!Conectar()) {
         return null;
  ...
  PreparedStatement psquery = con.prepareStatement(sql,ResultSet.TYPE_SCROLL_INSENSITIVE,
   ...
   while (rs.next()) {
       list.add(new Tema(
         rs.getString("id_tema"),rs.getString("nombre")));
    }
    rs.close();
    psquery.close();
    con.close();
