...
@ManagedBean
@SessionScoped
public class SalaPersistencia implements Serializable {
private static final long serialVersionUID = 1L;
public static Exception excepcion;
public static Connection con = null;

public SalaPersistencia(){
...
public static boolean Insertar (Sala sala ) {
	...
	String sql = "insert into sala(id_sala,id_biblioteca,nombre,ubicacion)  values('"+sala.getId_sala()+"','"+sala.getId_biblioteca()+"','"+sala.getNombre()+"','"+sala.getUbicacion()+"')";
...
public static List<Sala> getLista(Map parametros) {
  excepcion = null;
  try {
      if (!Conectar()) {
         return null;
  ...
  PreparedStatement psquery = con.prepareStatement(sql,ResultSet.TYPE_SCROLL_INSENSITIVE,
   ...
   while (rs.next()) {
       list.add(new Sala(
         rs.getInt("id_sala"),rs.getInt("id_biblioteca"),rs.getString("nombre"),rs.getString("ubicacion")));
    }
    rs.close();
    psquery.close();
    con.close();
