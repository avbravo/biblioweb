...
@ManagedBean
@SessionScoped
public class ServicioPersistencia implements Serializable {
...
public static boolean Insertar (Servicio servicio ) {
	...
	String sql = "insert into servicio(id_servicio,nombre)  values('"+servicio.getId_servicio()+"','"+servicio.getNombre()+"')";
...
public static List<Servicio> getLista(Map parametros) {
  excepcion = null;
  try {
      if (!Conectar()) {
         return null;
  ...
  PreparedStatement psquery = con.prepareStatement(sql,ResultSet.TYPE_SCROLL_INSENSITIVE,
   ...
   while (rs.next()) {
       list.add(new Servicio(
         rs.getInt("id_servicio"),rs.getString("nombre")));
    }
    rs.close();
    psquery.close();
    con.close();
